...
7. **電力、邏輯與生命週期優化 (v1.1.5)**:
   - **智慧刷新率 (Adaptive Ticking)**: 根據 App 狀態動態調整 (前台 50ms / 背景 1s / 螢幕關閉 5s 喚醒)。
   - **零喚醒音效 (Sound Scheduling)**: 移除高頻輪詢，改用 Coroutine 延遲預約音效播放，允許 CPU 在倒數期間深層休眠。
   - **健全的連線過渡**: 修正 PC 端關閉或連線中斷時的偵測邏輯，斷線後手機自動重置回預設狀態並取消所有預約。
   - **自動循環與修復**: 修正手動切換階段導致的暫停 Bug；實現離線模式下的自動階段循環。
   - **持久化簽名**: 導入固定 `debug.keystore` 並配置 Gradle，解決 APK 升級時簽名不一致無法更新的問題。
8. **可靠性與同步修復 (v1.1.6)**:
   - **修正螢幕關閉時間凍結**: 將時間基準 (`lastTime`) 全域化並縮短休眠間隔至 5 秒，確保背景與螢幕關閉時計時不中斷。
