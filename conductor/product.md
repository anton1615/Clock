...
*   **系統級預約與喚醒 (Android)**：音效觸發與階段轉場改由 `AlarmManager.setExactAndAllowWhileIdle` 絕對驅動。支援在黑畫面轉場時自動播放音效並**點亮螢幕 3 秒**。
*   **非同步轉場與音訊優化 (Android)**：轉場邏輯優先執行「階段切換」與「UI 刷新」，音效則透過 `prepareAsync()` 異步準備與播放。這解決了在播放音樂等音訊競爭環境下導致的轉場卡死（十幾秒）問題，確保計時器絕對精準。
