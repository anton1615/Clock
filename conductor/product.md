...
*   **智慧電力優化 (Android)**：導入 Adaptive Ticking 與 **系統級預約音效 (AlarmManager)**。在背景 (1s) 或螢幕關閉 (1s) 時採動態刷新。音效觸發改由 `AlarmManager.setExactAndAllowWhileIdle` 管理，繞過 Android Doze 模式對 App 協程的凍結限制，確保在省電的同時維持計時與同步的精準度，不因系統休眠而遺失時間或漏響。
*   **負數保護機制**：所有計時路徑均加入 Math.max(0.0) 箝位保護，徹底解決同步偏差導致的負數顯示問題。
*   **健全生命週期 (Android)**：優化服務退出機制，當使用者從任務列表滑掉 App 時，自動清理背景服務與通知，確保資源不被無謂佔用。
